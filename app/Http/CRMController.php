...
public function getRelances(Request $request)
{
    try {
        ...
        $clientsARelancer = CRMClient::whereDoesntHave('relances', function($q) {
            $q->where('date_relance', '>=', now()->startOfWeek())
              ->where('date_relance', '<=', now()->endOfWeek());
        })
        ->orWhereHas('relances', function($q) {
            $q->where('prochaine_relance', '<=', now())
              ->where('statut', 'En cours');
        })
        ->with('relances')
        ->get();
...
public function getRelancesStats()
{
    try {
        ...
        $stats = [
            'total' => $query->count(),
            'en_cours' => $query->where('statut', 'En cours')->count(),
            'clotures' => $query->where('statut', 'Clôturé')->count(),
            'cette_semaine' => $query->whereBetween('date_relance', [
                now()->startOfWeek(),
                now()->endOfWeek()
            ])->count(),
...
public function getRelancesAujourdhui()
{
    try {
        ...
        $relancesAujourdhui = CRMRelance::with('client')
            ...
            ->when(!$user->hasAnyRole(['Super Admin', 'Admin']), function($query) use ($user) {

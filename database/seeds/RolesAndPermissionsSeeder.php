...
{
    /**
     * ✅ AFFICHER LA LISTE DES RÔLES SANS DOUBLONS
     */
    ...
    /**
     * ✅ CORRECTION MAJEURE : API DataTables SANS AUCUN DOUBLON
     */
    public function getRoleList(Request $request)
    {
        try {
            Log::info('🚀 getRoleList: Début requête DataTables - CORRECTION DOUBLONS DÉFINITIVE');
            ...
            // ✅ SOLUTION DÉFINITIVE : Nettoyer d'abord les doublons existants
            $this->cleanDuplicateRoles();

            // ✅ REQUÊTE PRINCIPALE SANS DOUBLONS
            $roles = Role::select('id', 'name', 'guard_name', 'created_at', 'updated_at')
                ->groupBy('id', 'name', 'guard_name', 'created_at', 'updated_at')
                ->orderBy('name', 'asc')
                ->get()
                ->unique('name'); // Double sécurité pour éliminer les doublons
            
            Log::info("✅ getRoleList: {$roles->count()} rôles uniques récupérés");

            return Datatables::of($roles)
                ->addColumn('permissions', function ($role) {
                    return $this->formatPermissionsUnique($role);
                })
    ...
    /**
     * ✅ NOUVELLE MÉTHODE : Nettoyer les doublons de rôles
     */
    private function cleanDuplicateRoles()
    {
        try {
            Log::info('🧹 Nettoyage des doublons de rôles...');
            ...
            foreach ($duplicates as $duplicate) {
                ...
                // Garder le plus ancien (MIN id) et supprimer les autres
                ...
                if ($duplicateIds->count() > 0) {
                    // Supprimer les relations des doublons
                    DB::table('model_has_roles')->whereIn('role_id', $duplicateIds)->delete();
                    DB::table('role_has_permissions')->whereIn('role_id', $duplicateIds)->delete();
                    
                    // Supprimer les doublons
                    DB::table('roles')->whereIn('id', $duplicateIds)->delete();
                    
                    Log::info("✅ Supprimé {$duplicateIds->count()} doublons pour le rôle: {$duplicate->name}");
                }
            }

        } catch (\Exception $e) {
            Log::error('❌ Erreur nettoyage doublons: ' . $e->getMessage());
        }
    }

    /**
     * ✅ MÉTHODE CORRIGÉE : Formater les permissions SANS DOUBLONS
     */
    private function formatPermissionsUnique($role): string
    {
        try {
    ...
    /**
     * ✅ MÉTHODE PUBLIQUE : Nettoyer tous les doublons (à appeler manuellement si besoin)
     */
    public function cleanAllDuplicates()
    {
        try {
            Log::info('🧹 Démarrage nettoyage complet des doublons...');
            
            // Nettoyer les doublons de rôles
            $this->cleanDuplicateRoles();
            
            // Nettoyer les assignations de permissions en doublon
            $this->cleanDuplicateRolePermissions();
            
            // Nettoyer les assignations d'utilisateurs en doublon
            $this->cleanDuplicateUserRoles();
            
            return redirect('roles')->with('success', 'Nettoyage des doublons terminé avec succès !');
            
        } catch (\Exception $e) {
            Log::error('❌ Erreur nettoyage complet: ' . $e->getMessage());
            return redirect('roles')->with('error', 'Erreur lors du nettoyage: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            // Supprimer les doublons dans role_has_permissions
            DB::statement("
            ...
            Log::info('✅ Doublons de permissions de rôles nettoyés');
            
        } catch (\Exception $e) {
            Log::error('❌ Erreur nettoyage permissions rôles: ' . $e->getMessage());
        }
    }
    ...
    private function cleanDuplicateUserRoles()
    {
        try {
            ...
            Log::info('✅ Doublons d\'assignations utilisateur-rôle nettoyés');
            
        } catch (\Exception $e) {
            Log::error('❌ Erreur nettoyage assignations: ' . $e->getMessage());
        }
    }
    ...
    public function getStatistics()
    {
        try {
            $stats = [
                'total_roles' => Role::distinct('name')->count(),
                'roles_with_users' => Role::has('users')->distinct()->count(),
                'roles_without_users' => Role::doesntHave('users')->distinct()->count(),
                'total_permissions' => Permission::distinct('name')->count(),

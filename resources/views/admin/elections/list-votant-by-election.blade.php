<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table = $('#data_tableListeVotantByElection<?=$tabElections->id?>').DataTable({
            responsive: true,
            select: true,
            'aoColumnDefs': [{
                'bSortable': false,
        ...
        $('#data_tableListeVotantByElection<?=$tabElections->id?> tbody').on( 'click', 'tr', function() {
...
<table id="data_tableListeVotantByElection<?=$tabElections->id?>" class="table">
    <thead>
        <tr>
            <th>{{ __('Nom & Prénom')}}</th>
            <th>{{ __('Matricule')}}</th>
    ...
    <tbody>
        <?php
            foreach($dataElecteur as $tabUsers)
            {
        ?>
                <tr>
                    <td>

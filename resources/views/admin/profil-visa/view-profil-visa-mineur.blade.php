    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-justify">
                ...
                @endforeach
                @foreach(App\Models\InformationsParents::where('id_profil_visa', $tabProfilVisa->id)->get() as $tabInformationsParent)
                    <div class="card">
                        <div class="card-header bg-{{$bgColor}} text-white text-center">
                        ...
                        <div class="card-body">
                            <table class="table">
                                <tr>
                                    <th>Nom & Prenom du père</th>
                                    <td>{{$tabInformationsParent->nom_pere}} {{$tabInformationsParent->prenom_pere}}</td>
                                </tr>
                                <tr>
                                    <th>Adresse du père</th>
                                    <td>{{$tabInformationsParent->adresse_pere}}</td>
                                </tr>
                                <tr>
                                    <th>Profession du père</th>
                                    <td>{{$tabInformationsParent->profession_pere}}</td>
                                </tr>
                                <tr>
                                    <th>Email du père</th>
                                    <td>{{$tabInformationsParent->email_pere}}</td>
                                </tr>
                                <tr>
                                    <th>Numéro Whatsapp du père</th>
                                    <td>{{$tabInformationsParent->num_whatsapp_pere}}</td>
                                </tr>

                                <tr>
                                    <th>Nom & Prenom de la mère</th>
                                    <td>{{$tabInformationsParent->nom_mere}} {{$tabInformationsParent->prenom_pere}}</td>
                                </tr>
                                <tr>
                                    <th>Adresse de la mère</th>
                                    <td>{{$tabInformationsParent->adresse_mere}}</td>
                                </tr>
                                <tr>
                                    <th>Profession de la mère</th>
                                    <td>{{$tabInformationsParent->profession_mere}}</td>
                                </tr>
                                <tr>
                                    <th>Email de la mère</th>
                                    <td>{{$tabInformationsParent->email_mere}}</td>
                                </tr>
                                <tr>
                                    <th>Numéro Whatsapp  de la mère</th>
                                    <td>{{$tabInformationsParent->num_whatsapp_mere}}</td>
                                </tr>
                            </table>
                        </div>
                ...
                @endforeach
                @foreach(App\Models\InformationsMineurs::where('id_profil_visa', $tabProfilVisa->id)->get() as $tabInformationsMineurs)
                    <div class="card">
                        <div class="card-header bg-{{$bgColor}} text-white text-center">
                            INFORMATIONS MINEUR
                        </div>
                        <div class="card-body">
                            <table class="table">
                                <tr>
                                    <th>Niveau etude</th>
                                    <td>
                                        @if($tabInformationsMineurs->niveau_etude_mineur != 100)
                                            @foreach(App\Models\niveau_etude::where('id', $tabInformationsMineurs->niveau_etude_mineur)->get() as $TABniveau_etude_mineur)
                                                {{$TABniveau_etude_mineur->libelle}}
                                            @endforeach
                                        @else
                                            {{$tabInformationsMineurs->autre_niveau_etude_mineur}}
                                        @endif
                                    </td>
                                    <tr>
                                        <th>Passport</th>
                                        <td>{{$tabInformationsMineurs->passeport}}</td>
                                    </tr>
                                    <tr>
                                        <th>Pays Actuel</th>
                                        <td>
                                            
                                            @if($tabInformationsMineurs->pays_accueil != 100)
                                                @foreach(App\Models\Countries::where('id', $tabInformationsMineurs->pays_accueil)->get() as $TABpays_accueil)
                                                    {{$TABpays_accueil->libelle}}
                                                @endforeach
                                            @else
                                                {{$tabInformationsMineurs->autre_pays_accueil}}
                                            @endif
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Motif</th>
                                        <td>{{$tabInformationsMineurs->motif}}</td>
                                    </tr>
                                    <tr>
                                        <th>Classe</th>
                                        <td>{{$tabInformationsMineurs->classe_enfant}}</td>
                                    </tr>
                                    <tr>
                                        <th>Date migration</th>
                                        <td>{{$tabInformationsMineurs->date_migration}}</td>
                                    </tr>
                                    <tr>
                                        <th>Pays de résidence</th>
                                        <td>

                                            @if($tabInformationsMineurs->pays_residence != 100)
                                                @foreach(App\Models\Countries::where('id', $tabInformationsMineurs->pays_residence)->get() as $TABpays_residence)
                                                    {{$TABpays_residence->libelle}}
                                                @endforeach
                                            @else
                                                {{$tabInformationsMineurs->autre_pays_residence}}
                                            @endif
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Nom hôte</th>
                                        <td>{{$tabInformationsMineurs->nom_hote}}</td>
                                    </tr>
                                    <tr>
                                        <th>Nationalité Actuel</th>
                                        <td>{{$tabInformationsMineurs->nationalite_actuelle}}</td>
                                    </tr>
                                    <tr>
                                        <th>Lien Parenté du hôte avec l'enfant </th>
                                        <td>{{$tabInformationsMineurs->lien_parente_enfant}}</td>
                                    </tr>
                                    <tr>
                                        <th>Lien parenté  du hôte avec avec le parent </th>
                                        <td>{{$tabInformationsMineurs->lien_parente_enfant_parent}}</td>
                                    </tr>
                                </tr>
                                <?php /*
                                    <tr>
